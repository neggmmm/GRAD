...
const CreateChannel = () => {
  ...
  const handleButtonClick = async (e) => {
    ...
    const res = await fetch(
      'http://localhost:5000/api/admin/createChannel',
      {
        method: 'POST',
        headers: {
...
const Channels = () => {
  ...
  const fetchChannelsId = useCallback(async () => {
    const response = await fetch(
      'http://localhost:5000/api/channels/getAllChannels',
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
  ...
  const fetchChannelsData = useCallback(async () => {
    channelsId.forEach(async (channelId) => {
      const response = await fetch(
        `http://localhost:5000/api/channels/getChannelById/${channelId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  const isUserInChannel = useCallback(async (channelId) => {
    ...
    if (role === 'user') {
      const res = await fetch(
        `http://localhost:5000/api/users/isUserInChannel/${channelId}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleJoinChannel = useCallback(async (channel) => {
    console.log(channel)
    const res = await fetch(
      `http://localhost:5000/api/channels/joinChannel`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
